...
std::unique_ptr<FeatureDetector> FeatureDetector::build(
    ...
    // Use this to check the feature detector (p.featureDetector is a const so we need new var to overload it)
    std::string feature_detector_overloaded = p.featureDetector;

    /* PCL addition */
    if (SEAL->isKeypointDetectorOn()) {
        return buildLegacySEAL(w, h, p);
    }
    ...
    if (feature_detector_overloaded == "FAST")
        return buildLegacyFAST(w, h, p);

    if (feature_detector_overloaded == "GFTT")
