...
void computeImplementationCpu(
    ...
    /* PCL addition:
     * If SEAL's keypoint tracking is enabled use it,
     * otherwise use the HybVIO's default tracking algorithm.
     */
    if (SEAL->isKeypointTrackerOn()) {
        // Use SEAL's LK optical flow:
