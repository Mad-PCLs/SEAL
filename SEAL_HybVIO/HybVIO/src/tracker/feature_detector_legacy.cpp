...
/* PCL addition: Include SEAL as global variable. */
...
public:
    ...
    : MaskedFeatureDetector(imageWidth, imageHeight, params) {
        ...
        cv::Ptr<cv::FastFeatureDetector> d;

        d = cv::FastFeatureDetector::create();
...
/* PCL addition: Wrap SEAL's detector to a class compatible with HybVIO */
class SEALFeatureDetector : public MaskedFeatureDetector {
    public:
        SEALFeatureDetector(
            int imageWidth,
            int imageHeight,
    ...
    protected:
        void detectNonMasked(
            const cv::Mat& image,
            std::vector<cv::Point2f>& corners
        ) final {
            keypoints.clear();
            corners.clear();
            SEAL->detect_keypoints(
                image,
                keypoints,
                mask
            );
...
/* PCL addition: Comply to HybVIO's architectural design. */
std::unique_ptr<FeatureDetector> FeatureDetector::buildLegacySEAL(
    int w, int h, const odometry::ParametersTracker &p)
{
return std::unique_ptr<FeatureDetector>(new SEALFeatureDetector(w, h, p));
}
