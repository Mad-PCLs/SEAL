...
: winSize(winSize), maxLevel(maxLevel), criteria(criteria), flags(useInitialFlow ? (flags | cv::OPTFLOW_USE_INITIAL_FLOW) : flags),
    minEigThreshold(minEigThreshold), spatialDerivativeType(spatialDerivativeType),
    pyrDownFilterType(pyrDownFilterType), useInitialFlow(useInitialFlow)
{ }
